...
## Gapminder data set
The data is from gapminder.csv, located in [Final Project Folder](https://github.com/erkavale/Applied-Data-Science-s19) and is not from the gapminder set available in R.
...
### Summary of data
Gapminder contains six variables: Country, Year, life (life expectancy), population, income (per capita), and region. It has 41284 observations. Missing values are located in population (25817 N/A values) and income (2341 N/A values).  
...
**Now let us use regression analysis to see if income can predict life expectancy.**
...
The distribution of our response variable is left skewed and bimodal. Regression will probably not work on this variable but let us test to find out.  

```{r echo=FALSE}
#Regression model
gap_glm<-glm(Ave_life_exp ~ median_income, data=year_1950to2015, family = 'gaussian')
...
year_1950to2015 %>%
  ...
  geom_point()+
  geom_line(aes(x=median_income, y=predicted), data=gap_pred, color="red")+
